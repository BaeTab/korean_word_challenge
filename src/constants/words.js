...
import { ANSWERS_7 } from './answers7'
...
// 7칸: guessDict(대량 사전)이 전부 2음절이라 3음절 7칸 단어가 없음 → 큐레이션 목록만 사용.
export const WORD_LIST_7 = ANSWERS_7.filter((w) => isTypable(w) && decomposeWord(w).length === 7)
...
export function preloadGuessDict() {
  ...
  _loading = import('./guessDict').then((m) => {
    ...
    for (const w of ANSWERS_7) {
      const k = key(w)
      if (k.length === 7) _set7.add(k)
    }
  })
  return _loading
}
...
export function isValidGuess(jamoArr, slots) {
  ...
  const set = slots === 7 ? _set7 : slots === 6 ? _set6 : _set5
...
/**
 * 모드별(5|6|7) 랜덤 정답 1개 선택.
 * @param {number} slots 5, 6 또는 7
 * @param {string} [exclude] 직전 정답(연속 중복 방지)
 */
export function pickRandomWord(slots, exclude) {
  const base = slots === 7 ? WORD_LIST_7 : slots === 6 ? WORD_LIST_6 : WORD_LIST_5
...
if (import.meta.env.DEV) {
  ...
  console.info(`[words] 정답 5칸 ${WORD_LIST_5.length} · 6칸 ${WORD_LIST_6.length} · 7칸 ${WORD_LIST_7.length} (추측사전은 지연 로드)`)
}
